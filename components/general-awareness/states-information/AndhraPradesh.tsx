...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-4">
      <h2 className="text-lg font-bold tracking-tight text-emerald-50 sm:text-xl">{children}</h2>
      <div
    ...
    <dl className="divide-y divide-slate-600/40 rounded-xl border border-slate-600/45 bg-slate-900/25">
      {items.map((row) => (
        <div key={row.label} className="grid gap-1 px-4 py-3.5 sm:grid-cols-[minmax(0,14rem)_1fr] sm:gap-4">
          <dt className="font-semibold text-emerald-200/95">{row.label}</dt>
  ...
  return (
    ...
    >
      {facts.map((fact) => (
        ...
        >
          <dt className="font-semibold text-emerald-200/95">{fact.label}</dt>
    ...
    <div className="relative min-h-screen overflow-hidden bg-linear-to-b from-slate-950 via-slate-900 to-emerald-950/80">
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        <header className="mb-10 text-center sm:mb-12">
          ...
          <h1 className="mt-3 bg-linear-to-r from-emerald-100 via-teal-100 to-emerald-200 bg-clip-text text-xl font-bold tracking-tight text-transparent sm:text-2xl md:text-3xl">
            ...
            <div className="space-y-8">
              {AP.miscellaneousBlocks.map((block, blockIndex) => (
                <div key={`${blockIndex}-${block.heading}`}>
                  <h3 className="mb-3 text-base font-bold text-emerald-100/95 sm:text-lg">{block.heading}</h3>
